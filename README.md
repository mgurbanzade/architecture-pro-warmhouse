...
**Упрощения MVP относительно целевой системы**

| В MVP | В целевой системе (задания 2 и 4) | Статус |
|---|---|---|
| Один инстанс PostgreSQL с отдельными базами `devices` и `telemetry` | Отдельный инстанс БД у каждого сервиса | временно, до выделения инфраструктуры под сервисы |
| Обычный PostgreSQL для телеметрии | TimescaleDB | временно, до появления истории и агрегатов на реальной нагрузке |
| Команды публикуются в топик `devices.commands` | Сервис устройств вызывает `POST /commands` шлюза устройств по REST | временно, до появления шлюза |
| Статус `online` выводится из показаний `telemetry.measurements` | Отдельный канал `devices.status` от шлюза | временно, до появления шлюза |
| temperature-api публикует показания в Kafka напрямую; идентификатор устройства в событиях — серийный номер | Шлюз устройств принимает MQTT/HTTP, нормализует и публикует; в событиях — uuid устройства | временно, до появления шлюза |
